-- GabLuchi injector stub.
-- The real backend is the GabLuchi GUI desktop app (HTTP on 127.0.0.1:6767). This lua
-- backend has two jobs:
--   1. Put gabluchi.js onto the Steam store webkit context (copy into steamui/webkit +
--      Millennium.add_browser_js) — delivery is backend-only in Millennium.
--   2. Be the RPC bridge between the injected page and the app. gabluchi.js calls
--      window.Millennium.callServerMethod("gabluchi", "<Name>", args) — under real
...
-- GabLuchi GUI's CefInjectorService, which polls the injected page over CDP and makes
-- the same HTTP calls from the app process itself. Any new RPC method added to
-- gabluchi.js needs a handler in BOTH places, or it only works under one loader.
...
-- Mirrors what the non-Millennium DLL hijack does (launch_gabluchi() in
...
local function ensure_backend_running()
    ...
    if not local_appdata or local_appdata == "" then
        logger.warn("LOCALAPPDATA not available, cannot launch GabLuchi backend")
        return
    end

    local exe_path = local_appdata .. "\\GabLuchi\\current\\GabLuchi.exe"
    if not fs.exists(exe_path) then
        logger.warn("GabLuchi.exe not found at " .. exe_path .. " (not installed?)")
        return
    end

    -- `start` launches detached and returns immediately; utils.exec only blocks on
    -- that, not on GabLuchi.exe itself.
    m_utils.exec('start "" "' .. exe_path .. '" --minimized')
    logger.log("Launched GabLuchi backend: " .. exe_path)
end

-- ── RPC handlers (must be GLOBAL functions — Millennium looks these up by name) ─

function HasGabLuchiForApp(appid)
    return backend_request("GET", "/has/" .. tostring(appid))
end

function DeleteGabLuchiForApp(appid)
...
function StartGabLuchiAdd(appid)
    return backend_request("POST", "/add/" .. tostring(appid))
end

function GetGabLuchiAddStatus(appid)
    return backend_request("GET", "/add-status/" .. tostring(appid))
end

function PickGabLuchiAddSource(appid, source)
...
local function copy_webkit_files()
    ...
    local src_js = fs.join(public_dir, "gabluchi.js")
    local dst_js = fs.join(target_webkit_dir, "gabluchi.js")
...
local function inject_webkit_files()
    ...
    millennium.add_browser_js("webkit/gabluchi.js")
end

-- ── Lifecycle ────────────────────────────────────────────────────────────────

local function on_load()
    logger.log("GabLuchi injector stub loading (millennium " .. tostring(millennium.version()) .. ")")
...
local function on_unload()
    logger.log("GabLuchi injector stub unloading")
end

local function on_frontend_loaded()
    -- Re-copy so an updated gabluchi.js is picked up without a manual step.
    copy_webkit_files()
end
